...
import { InsertNoteBook, Note, NoteBook, notebooks } from "@/db/schema";
...
export const getNoteBooks = async () => {
  try {
    ...
    return {
      success: true,
      notebooks: notebooksByUser as (NoteBook & { notes: Note[] })[],
    };
  } catch(error) {
    return { success: false, message: `Failed to get notebooks. Error -${error}` };
  }
};
